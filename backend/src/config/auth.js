...
=======
const JWT_SECRET = process.env.JWT_SECRET || 'nexus360-secret-key';
const JWT_EXPIRES_IN = '24h';
const SALT_ROUNDS = 12;
...
=======
  if (!user || !user.id || !user.username) throw new Error('Invalid user data');
  return jwt.sign({ id: user.id, username: user.username, role: user.role || 'viewer' }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
};
...
module.exports = { hashPassword, verifyPassword, generateToken, verifyToken, JWT_SECRET };
>>>>>>> d7011e7 (Initial commit: NEXUS360 complete platform)
