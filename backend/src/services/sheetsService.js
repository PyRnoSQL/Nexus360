...
class SheetsService {
  static async appendData(sheetName, values) {
    ...
    } catch (error) { throw new Error(`Failed to append data to ${sheetName}`); }
  }
  ...
  static async getData(sheetName) {
    ...
    try {
      ...
      return rows.slice(1).map((row, index) => {
        ...
        headers.forEach((header, colIndex) => { obj[header.toLowerCase()] = row[colIndex] || null; });
        return obj;
      });
    } catch (error) { throw new Error(`Failed to read data from ${sheetName}`); }
  }
  ...
  static async updateCell(sheetName, cell, value) {
    ...
    try {
      return await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID, range: `${sheetName}!${cell}`,
        valueInputOption: 'USER_ENTERED', resource: { values: [[value]] }
      });
    } catch (error) { throw new Error(`Failed to update cell ${cell}`); }
  }

  static async getLastId(sheetName, idColumn = 'A') {
    if (!isSheetsReady()) throw new Error('Google Sheets not configured');
    try {
      const response = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${sheetName}!${idColumn}:${idColumn}` });
